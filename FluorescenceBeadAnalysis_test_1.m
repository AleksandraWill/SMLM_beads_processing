%% Main function
function FluorescenceBeadAnalysis_test_1()
    % Main function to process fluorescence beads from z-stack images

    % Ask user for the file path
    [fileName, filePath] = uigetfile({'*.tif', 'TIFF Files (*.tif)'; '*.*', 'All Files (*.*)'}, ...
        'Select the z-stack TIFF file containing fluorescence beads');
    if fileName == 0
        disp('No file selected. Exiting.');
        return;
    end
    zStackFile = fullfile(filePath, fileName);
